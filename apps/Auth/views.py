...
from apps.Auth.serializers import ChangePasswordSerializer, UserSerializer
...
class Register(APIView):
    ...
    def post(self, request):
        ...
        if not (first_name and last_name and phone_number):
            return Response(
                {"msg": REQUIRED_FIELDS}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {
                    "msg": str(e),  # Return validation error message
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        phone_regex = re.compile(r'^\+?1?\d{9,15}$')  # Customize the regex pattern according to your requirements
